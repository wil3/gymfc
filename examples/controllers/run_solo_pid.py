...
class PIDPolicy(Policy):
    def __init__(self):
        ...
        self.y = [4, 50, 0.0]
        """
        self.r = [0.3, 1.2, 0.005]
        self.p = [0.3, 1.2, 0.005]
        self.y = [0.3, 1.2, 0.0]
    ...
    def action(self, state, sim_time=0, desired=np.zeros(3), actual=np.zeros(3) ):
       ...
       # return np.array( [ (m - 1000)/500  - 1 for m in motor_values])
        return (motor_values - 1000)/1000.0
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', help='Alternate location for config file', default="../configs/iris.config")
